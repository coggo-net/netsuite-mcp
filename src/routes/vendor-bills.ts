...
export function vendorBillRoutes(api: VendorBillAPI): RouteDef[] {
	return [
		...
		defineRoute({
			...
			description:
				'Create a new vendor bill (supplier invoice) in NetSuite. Lot/serial-tracked lines: vendor bills are INBOUND, so set inventoryAssignment.items[].receiptInventoryNumber to the lot/serial NUMBER STRING (e.g. "SBLF2672") — NOT an {id} or {refName} object. inventoryDetail.quantity MUST equal the line quantity and the sum of inventoryAssignment.items[].quantity MUST also equal it. Note: many accounts block inline lot creation on standalone vendor bills (NetSuite returns INVALID_VALUE on receiptInventoryNumber). When that happens, switch to the standard inbound flow: purchase_order_create → purchase_order_receive (lot is born on the receipt) → vendor_bill_create_from_po.',
			body: vendorBillBody,
			successStatus: 201,
