...
export function vendorBillRoutes(api: VendorBillAPI): RouteDef[] {
	return [
		...
		defineRoute({
			...
			description:
				"Create a new vendor bill (supplier invoice) in NetSuite. Lot/serial-tracked lines: vendor bills are INBOUND, so use receiptInventoryNumber per inventoryAssignment item. Pass {refName: '...'} to auto-create a new lot, or {id: '...'} to bill against an existing one. inventoryDetail.quantity MUST equal the line quantity and the sum of inventoryAssignment.items[].quantity MUST also equal it. Auto-fallback: if NetSuite rejects inline {refName} with INVALID_VALUE / USER_ERROR (some account configs disallow inline lot creation on standalone vendor bills), this endpoint will automatically pre-create each new lot via inventoryNumber and retry with {id}. If pre-create is ALSO blocked ('cannot create a standalone inventory number record'), the endpoint surfaces a clear error pointing to the PO → Item Receipt → Vendor Bill workflow as the only viable path on that account.",
			body: vendorBillBody,
			successStatus: 201,
